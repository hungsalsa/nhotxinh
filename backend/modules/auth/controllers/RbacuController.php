...
//         $auth->add($categories_delete);
    ...
    public function actionIndex()
    {
        ...
        $user_index = $auth->createPermission('backend/user/index');
        $user_index->description = 'Danh sách Account';
        $auth->add($user_index);

        $user_signup = $auth->createPermission('backend/user/signup');
        $user_signup->description = 'Thêm mới Account';
        $auth->add($user_signup);

        $user_view = $auth->createPermission('backend/user/view');
        $user_view->description = 'Chi tiết Account';
        $auth->add($user_view);

        $user_update = $auth->createPermission('backend/user/update');
        $user_update->description = 'Chỉnh sửa Account';
        $auth->add($user_update);

        $user_delete = $auth->createPermission('backend/user/delete');
        $user_delete->description = 'Xóa Account';
        $auth->add($user_delete);

        $user_Resetpassword = $auth->createPermission('backend/user/resetpassword');
        $user_Resetpassword->description = 'Reset Password Account';
        $auth->add($user_Resetpassword);

        $user_Changepassword = $auth->createPermission('backend/user/changepassword');
        $user_Changepassword->description = 'Changepassword Account';
        $auth->add($user_Changepassword);


        $auth->addChild($manager, $user_index);
        $auth->addChild($manager, $user_view);
        $auth->addChild($manager, $user_Resetpassword);
